...
# group comfy asym_w4a8_int8 checkpoints ingest with, and cb4 auto resolves to half the uint4 group
...
dynamic_quant_requests = ["uint2", "uint4", "int6", "cb4"] # dynamic quantization escalates until loss passes; the cb entry checks the gated ladder
...
def layer_storage_bytes(layer):
    ...
    # zero points, codebook and svd factors; sizes on disk and in vram follow this, not the nominal bits
    total = 0
    for name in ("weight", "scale", "zero_point", "svd_up", "svd_down", "codebook"):
...
def codebook_kwarg(layer):
    # the shared-codebook dtypes carry one more tensor the dequantizer needs; stock sdnq has no such
    # keyword, so it is passed only when the layer carries it
        ...
        for index, (dtype_id, label, cfg) in enumerate(dtype_configs, start=1):
            ...
            def eager_fn(current=layer):
                return current.sdnq_dequantizer(
                    ...
                    svd_up=getattr(current, "svd_up", None), svd_down=getattr(current, "svd_down", None), **codebook_kwarg(current),
                    skip_compile=True,
                )
            ...
            try:
                ...
                with time_limit(config_timeout, row_label):
                    deq_out = layer.sdnq_dequantizer(
                        ...
                        zero_point=getattr(layer, "zero_point", None),
                        svd_up=getattr(layer, "svd_up", None), svd_down=getattr(layer, "svd_down", None), **codebook_kwarg(layer),
                    ).to(torch.float32)
                    fwd_fn()
                    torch_device_module.synchronize()
                entry["weight_err"] = rel_err(deq_out, weight_fp32)
