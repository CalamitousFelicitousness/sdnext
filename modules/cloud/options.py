...
def register_options(options_templates: dict) -> None:
    options_templates.update(options_section(('cloud_options', "Cloud Providers"), {
        ...
        "cloud_default_image_provider":  OptionInfo('google', "Default image provider", gr.Dropdown,
            lambda: {"choices": list_providers('image', only_enabled=False) or ['google']}),
        "cloud_default_video_provider":  OptionInfo('google', "Default video provider", gr.Dropdown,
            lambda: {"choices": list_providers('video', only_enabled=False) or ['google']}),
