...
class ApiV2Config(BaseSettings):
    ...
    storage_warm_path: Path = Field(
        Path("outputs/v2/warm"), env="SDNEXT_V2_STORAGE_WARM"
    )
    storage_cold_path: Path = Field(
        Path("outputs/v2/cold"), env="SDNEXT_V2_STORAGE_COLD"
    )
