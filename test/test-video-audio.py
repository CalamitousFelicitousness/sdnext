...
def test_rate_prefers_the_processing_object():
    got = with_model(FakeModel(24000), lambda: video_save.get_audio_rate(FakeP(48000), default=16000))
    assert got == 48000, f'expected the request rate, got {got}'


def test_rate_falls_back_to_the_vocoder():
    got = with_model(FakeModel(44100), lambda: video_save.get_audio_rate(FakeP(), default=16000))
    assert got == 44100, f'expected the vocoder rate, got {got}'


def test_rate_falls_back_to_the_default():
    got = with_model(None, lambda: video_save.get_audio_rate(None, default=16000))
    assert got == 16000, f'expected the default, got {got}'


def test_rate_is_an_int():
    got = with_model(FakeModel(48000.0), lambda: video_save.get_audio_rate(None, default=24000))
