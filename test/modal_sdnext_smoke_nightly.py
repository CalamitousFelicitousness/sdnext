"""Run an sdnext smoke on a Modal GPU with torch nightly (cu132).

Nightly variant of modal_sdnext_smoke.py: torch/torchvision/torchaudio come from
the pytorch nightly index, installed before sdnext's installer runs, so the
installer accepts them as-is and never replaces them. The baked nightly is
whatever the index served at image-build time; rebuild the image to bump it.

Usage:
    modal run modal_sdnext_smoke_nightly.py                  # generate, save smoke.png locally
    modal run modal_sdnext_smoke_nightly.py --payload my.json
    modal run modal_sdnext_smoke_nightly.py::ui              # tunnel the web UI for interactive use
    modal run modal_sdnext_smoke_nightly.py --image a.png --image2 b.png --prompt "..."   # instruct edit with reference images

Shares the 'sdnext-models' volume with the stable variant, so weights download once.
"""
...
app = modal.App("sdnext-smoke-nightly")
...
image = (
    ...
    # torch nightly goes in first; sdnext's installer never replaces an installed torch.
    # NIGHTLY_TAG busts the layer cache: NIGHTLY_TAG=2026-08-20 modal run ... rebakes today's nightly
    .run_commands(f"echo {os.environ.get('NIGHTLY_TAG', 'nightly')} && uv pip install --system --pre torch torchvision torchaudio --index-url https://download.pytorch.org/whl/nightly/cu132")
    .run_commands(f"git clone --depth 1 -b {BRANCH} {REPO_URL} {REPO_DIR}")
    # bake the remaining dependencies around the preinstalled torch;
    # the trailing test generation has no model and may fail, deps are in by then
    .run_commands(f"cd {REPO_DIR} && python launch.py --debug --test --uv || true", gpu="T4")
