...
def detect_comfy_quant(state_dict: dict, type_name: str) -> tuple[dict[str, dict], str] | None:
    ...
    supported = [fmt for fmt, weights_dtype in COMFY_QUANT_FORMATS.items() if weights_dtype in dtype_dict] # a format is only usable when the sdnq build carries its dtype
    unsupported = sorted(formats - set(supported))
    if unsupported:
        log.error(
